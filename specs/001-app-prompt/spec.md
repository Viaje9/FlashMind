...
- Q: 牌組內發現使用者新增重複單字時應如何處理？ → A: 建議避免但允許，並同步進度
- Q: 匿名使用者登入帳號時本地資料與雲端資料應如何合併？ → A: 以本地匿名資料覆蓋雲端內容
...
- 同一牌組內新增重複單字時，系統需提示內容已存在並說明將沿用原卡片排程與進度，若使用者確認仍要新增則維持單一卡片並更新其內容。

## 假設

- 匿名使用者在首次啟動時自動建立預設牌組，登入後以本地匿名資料覆蓋雲端既有內容並於同步時處理重複卡片。
...
- **FR-011**：系統必須支援匿名模式與登入同步；登入後需以本地匿名資料覆蓋雲端既有內容，同步所有牌組、卡片與排程並記錄覆蓋歷程。
...
- **FR-017**：當使用者在同一牌組內新增已存在的單字時，系統須顯示重複提示並提供「沿用既有卡片」選項；若使用者選擇沿用，需保留原排程與進度並允許更新定義與例句。
