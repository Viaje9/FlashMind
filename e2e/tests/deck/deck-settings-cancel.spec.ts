...
test.describe('編輯牌組設定', () => {
  test('測試取消修改', async ({ page, request }) => {
    // 1. 使用 API 建立測試帳號並登入
    ...
    // 2. 透過 API 建立測試牌組「取消測試」
    const deckResponse = await request.post('http://localhost:3280/decks', {
      data: { name: '取消測試', dailyNewCards: 20, dailyReviewCards: 100 }
    });
    ...
    await expect(page.getByRole('heading', { name: '取消測試', level: 1 })).toBeVisible();
