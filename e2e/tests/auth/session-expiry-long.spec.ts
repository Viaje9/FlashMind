...
test.describe('Session 與權限管理', () => {
  test.skip('驗證長效期 Session（勾選記住我）', async ({ page, request, context }) => {
    const testEmail = 'test-session-long@example.com';
    const testPassword = 'password123456';

    // 前置條件：使用 Email 登入，勾選「記住我」
    ...
    // 3. 驗證 Session cookie 的 Max-Age 為 30 天（2592000 秒）
    // 註：實際測試時需要檢查 cookie 的過期時間
    // 此處假設 30 天的 session 期限
