...
export class SpeakingLocalRepository {
  ...
  async deleteConversationAudio(conversationId: string): Promise<void> {
    if (!this.isIndexedDbAvailable()) return;
    const db = await this.openDb();
    const tx = db.transaction(AUDIO_STORE, 'readwrite');
    const store = tx.objectStore(AUDIO_STORE);
    const records = await this.requestToPromise<SpeakingAudioRecord[]>(
      store.index('byConversationId').getAll(IDBKeyRange.only(conversationId)),
    );
  ...
  async enforceConversationStorageLimit(
    ...
    const candidates = [...conversations]
      .filter((item) => item.id !== protectedConversationId && !item.syncPending)
    ...
    for (const conversation of candidates) {
      ...
      await this.deleteConversationAudio(conversation.id);
