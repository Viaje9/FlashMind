...
export class AgentsCollectionAiProvider extends CollectionAiProvider {
  ...
  private buildPrompt(input: CollectionAiChatInput) {
    ...
    return [
      ...
      'suggestedCards 規則：只放「完成這句自然英文真的需要、值得學、且使用者單字卡中找不到」的核心單字或短語。搭配詞、片語、子句不要放入 candidates；如果它值得學，請改放到 suggestedCards 當作 front 為短語的建議單字卡。不可為了湊數建議冠詞、介系詞、代名詞、be 動詞、助動詞等功能字，也不可建議已出現在單字卡樣本或相關單字卡中的字。',
      ...
      '缺字候選一致性規則：如果 message 或 sentence candidate 中出現使用者卡片找不到的核心名詞、動詞或形容詞，suggestedCards 不可為空；請加入最核心的 1 到 3 個缺字。',
      ...
      'candidates 規則：只回 sentence。不要回 collocation、phrase、clause，也不要在 relatedCandidates 放任何項目；每個 sentence candidate 的 relatedCandidates 必須是空陣列。',
      '短語建議規則：如果像 postpone the meeting、no sauce、be full 這種短語值得學，請放入 suggestedCards，front 就是該短語，meanings 填中文意思與例句。不要把短語做成可收藏表達卡。',
