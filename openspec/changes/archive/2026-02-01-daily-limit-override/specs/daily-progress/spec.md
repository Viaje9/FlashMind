...
系統 SHALL 在牌組詳情頁的統計卡片及牌組列表頁的卡片中顯示今日練習進度。進度的分母 SHALL 使用有效每日上限（考慮覆寫值）。

#### Scenario: 顯示新卡進度

- **WHEN** 使用者進入牌組詳情頁或牌組列表頁
- **THEN** 統計卡片的「新卡片」數字下方 SHALL 顯示今日新卡進度
- **AND** 進度格式為 `N / M`（N = 今日已學新卡數，M = 有效每日新卡上限）
...
- **AND** 進度格式為 `N / M`（N = 今日已複習數，M = 有效每日複習上限）
- **AND** SHALL 顯示進度條，寬度比例為 N / M

#### Scenario: 進度已達上限

- **WHEN** 今日已學數量等於或超過有效每日上限
...
- **AND** `dailyNewCards` 與 `dailyReviewCards` SHALL 為有效上限值（考慮覆寫）
