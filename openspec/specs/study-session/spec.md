...
系統 SHALL 提供開始學習功能，讓使用者取得今日應學習的卡片。若牌組啟用反向學習，SHALL 同時回傳正向與反向 StudyCard，並根據學習日起始時間扣除今日已學額度。
...
- **WHEN** 牌組啟用反向學習
- **THEN** 正向與反向 SHALL 共用同一份 dailyNewCards 額度
- **AND** 正向與反向 SHALL 共用同一份 dailyReviewCards 額度
